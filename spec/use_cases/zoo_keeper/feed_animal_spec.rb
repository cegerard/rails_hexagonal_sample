...
describe UseCases::ZooKeeper::FeedAnimal do
  ...
  context 'when the animal to feed is carnivor' do
    ...
    let(:animal) { Core::Animal::Animal.new(eat_behaviour, health_behaviour) }

    context 'when the keeper give meat' do
      let(:meat) { Core::Food::Food.new(:meat, 80.0) }
      let(:empty_food) { Core::Food::EmptyFood.new }
    ...
    context 'when the keeper give vegetables' do
      let(:vegetables) { Core::Food::Food.new(:vegetable, 30.0) }
      let(:not_eaten_vegetables) { Core::Food::Food.new(:vegetable, 27.0) }
  ...
  context 'when the animal to feed is herbivor' do
    let(:eat_behaviour) { Core::Animal::Behaviours::VegetarianEatBehaviour.new }
    let(:animal) { Core::Animal::Animal.new(eat_behaviour, health_behaviour) }
